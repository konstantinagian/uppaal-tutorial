...
Η λάμπα μπορεί επομένως να βρίσκεται σε δύο καταστάσεις, off και on, οι οποίες αναπαριστούνται από δυο **τοποθεσίες (locations)** στο μοντέλο μας. Χρειάζεται να ορίσουμε και ένα δεύτερο μοντέλο, αυτό του χρήστη για να μοντελοποιήσουμε το πάτημα του κουμπιού. Αυτό αποτελείται από μια τοποθεσία. Οι τοποθεσίες ενώνονται με **ακμές (edges)**.
...
Αντίστοιχα, στις τοποθεσίες μπορείς να ορίσεις invariants αλλά και να επιλέξεις ανάμεσα σε ειδικούς τύπους τοποθεσιών: initial(αρχική), committed και urgent.
...
Το ρολόι πρέπει να μηδενίζεται για να ξεκινάει η χρονομέτρηση κάθε φορά που ανάβει η λάμπα. Γι'αυτό κάνουμε χρήση του update της ακμής από το on στο off. Για να βεβαιωθούμε ότι η λάμπα δε θα μείνει αναμμένη πάνω από 30 χρονικές μονάδες ορίζουμε ένα invariant t <= 30 στην τοποθεσία on.
...
Τα update είναι εκφράσεις (expressions) που αποτιμούνται όταν το αυτόματο ακολουθεί αυτή την ακμή. Συνήθως, πρόκεται για αναθέσεις σε μεταβλητές όπως το ρολόι στην περίπτωσή μας.

Τα invariants είναι boolean εκφράσεις της μορφής t <= e ή τ < e όπου t είναι ένα ρολόι και e ένας ακέραιος. Πρακτικά, ορίζουν το μέγιστο χρόνο που το αυτόματο μπορεί να παραμείνει στο συγκεκριμένο location.
...
- `A[] not (Lamp.on and t > 30)` δηλαδή οτί δεν υπάρχει κατάσταση του συστήματος όπου μπορεί να είναι η λάμπα αναμμένη (το αυτόματο της λάμπας στην τοποθεσία on) και το ρολόι να είναι μετά το 30. Είναι αληθές.
