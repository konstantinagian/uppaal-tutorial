...
Έστω πως ένας οδηγός θέλει να φτάσει από την αρχική κατάσταση του *start* στον τελικό προορισμό του *goal*. Υπάρχουν δυο διαδρομές που μπορεί να ακολουθήσει είτε κυκλικά εκτός της πόλης (*go_around*), είτε περνόντας μέσα από την πόλη (*go_through*) όπου όμως θα συναντήσει φανάρι.
...
Αρχικά οι **συνθήκες μετάβασης (guards)** είναι άλλο ένα χαρακτηριστικό των ακμών και ορίζει ότι η μετάβαση είναι δυνατή αν και μόνο αν η συνθήκη είναι αληθής. Εδώ, βλέπουμε guards στις μεταβάσεις από τις καταστάσεις *go_around*, *green*, *red* στο *goal*.

### Committed και urgent καταστάσεις

Επιπλέον, βλέπουμε ότι οι καταστάσεις *start* και *go_through* έχουν τους ειδικούς τύπους **committed** και **urgent** αντίστοιχα. Σε αυτούς τους τύπους καταστάσεων δεν "επιτρέπεται" να περάσει χρόνος και πρέπει να ακολουθηθεί μια μετάβαση άμεσα. Στο παράδειγμά μας μπορούμε να το καταλάβουμε διαισθητικά σκεπτόμενοι ότι δεν θα χρειαστεί χρόνος να αποφασίσει ο οδηγός ανάμεσα στις *go_around* και *go_through*. Επίσης, ότι μόλις φτάσει στο φανάρι θα είναι άμεσα σε μια από τις δύο καταστάσεις, πράσινο ή κόκκινο.
...
- `Pr[<=10](<> Process.goal)` δηλαδή ποιά είναι η πιθανότητα να φτάσουμε στην κατάσταση *goal* σε λιγότερες ή 10 χρονικές μονάδες. Παίρνουμε αποτέλεσμα 0.37342 ± 0.0489831  (95%  CI)
- `Pr[<=30](<> Process.goal)` δηλαδή ποιά είναι η πιθανότητα να φτάσουμε στην κατάσταση *goal* σε λιγότερες ή 30 χρονικές μονάδες. Παίρνουμε αποτέλεσμα 0.921013 ± 0.0481761 (95%  CI)
- `Pr[<=100](<> Process.goal)` δηλαδή ποιά είναι η πιθανότητα να φτάσουμε στην κατάσταση *goal* σε λιγότερες ή 100 χρονικές μονάδες. Παίρνουμε αποτέλεσμα  ≥0.950056  (95%  CI)
...
Λογικό είναι τώρα να θέλουμε μια εκτίμηση του συνολικού χρόνου που χρειάζεται ο οδηγός για να φτάσει στον προορισμό του. Θα ορίσουμε ένα νέο ρολόι `trip` το οποίο θα σταματάμε όταν φτάνουμε στο *goal*. Αυτό το κάνουμε γιατί το ρολόι συνεχίζει να προχωράει για όσο τρέχει η προσομοίωση. Εμείς θέλουμε όμως να σταματήσει μόλις η προσομοίωση φτάσει στο *goal*. Αυτό γίνεται ορίζοντας invariant `trip' == 0` σε εκείνη την κατάσταση. Πρόκειται για άλλη μια δυνατότητα του UPPAAL να αλλάζουμε το ρυθμό του ρολογίου τον οποίο στη συγκεκριμένη περίπτωση μηδενίζουμε ώστε να το σταματήσουμε.
