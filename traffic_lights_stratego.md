...
### Controllable και uncontrollable μεταβάσεις
...
Χρειάζεται πρακτικά να ορίσουμε uncontrollable και τις 3 μεταβάσεις προς το goal ακόμα και αν στη συγκεκριμένη περίπτωση υπάρχει μόνο μια μετάβαση ανάμεσα στις δυο καταστάσεις. Οι ακμές στη διακλάδωση είναι εξ ορισμού ήδη uncontrollable αφού είναι πιθανοτικές.
...
Έστω ότι αλλάζουν οι προτεραιότητες μας και πλέον θέλουμε με ασφάλεια να φτάνουμε στον προορισμό μας χωρίς να ξεπερνάμε τα 25 λεπτά. Ορίζουμε μια νέα στρατηγική με το όνομα Safest.

`strategy Safest = control: A<> (Process.goal && Process.x <= 25)`

Στο μοντέλο μας, η ασφαλής επιλογή πρέπει να είναι το *go_around*, επειδή η διαδρομή αυτή ολοκληρώνεται μεταξύ 20 και 25 χρονικών μονάδων. Αντίθετα, το *go_through* μπορεί να οδηγήσει στο *red*, όπου η διάρκεια μπορεί να φτάσει τις 60 μονάδες.

Μπορούμε να παρατηρήσουμε ότι αυτό συμβαίνει χρησιμοποιώντας τον Concrete Simulator του UPPAAL. Όταν δημιουργούμε μια νέα στρατηγική γίνεται διαθέσιμη για να τις προσομοιώσεις στον simulator, όπως φαίνεται στην εικόνα παρακάτω.

![concrete simulator](images/simulator.png)

*Figure 2. Concrete simulator.*

Το επιβεβαιώνουμε και στατιστικά τρέχοντας τα ίδια queries πλέον `under Safest`.

|| χωρίς στρατηγική    | με τη στρατηγική Fastest | με τη στρατηγική Safest |
| ------- | -------- | ------- | ----- |
| `Pr[<=30](<> Process.goal)` | 0.90 ± 0.04 | 0.81 ± 0.04 | ≥ 0.95 |
| `E[<=100;500](max: Process.trip)` | 19.23 ± 0.98 | 15.21 ± 1.27| 22.49 ± 0.12 |
